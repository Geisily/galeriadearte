  ...
  if (items.length === 0) {
    return (
      <main className="max-w-7xl mx-auto px-6 py-32 text-center">
        ...
        <Link to="/galeria" className="text-sm text-amber-500 hover:underline tracking-wider uppercase">
          Explorar galeria
        </Link>
      </main>
    )
    ...
    <main className="max-w-4xl mx-auto px-6 py-16">
        ...
        {items.map(({ artwork }) => (
          <div key={artwork._id} className="flex gap-6 py-6">
            ...
            <div className="flex-1">
              ...
              <p className="text-amber-400 mt-2">R$ {artwork.price.toLocaleString('pt-BR')}</p>
            </div>
      ...
      <div className="mt-8 flex items-center justify-between border-t border-neutral-800 pt-8">
        ...
        <div className="flex gap-4">
          ...
          <Link
            to="/checkout"
            className="px-8 py-3 bg-amber-700 text-white text-sm tracking-widest uppercase hover:bg-amber-600 transition-colors"
          >
            Finalizar Compra
          </Link>
        </div>
