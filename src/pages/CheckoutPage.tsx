  ...
  if (submitted) {
    return (
      <main className="max-w-2xl mx-auto px-6 py-32 text-center">
        <div className="w-12 h-12 border border-amber-600 rounded-full flex items-center justify-center mx-auto mb-6">
    ...
    <main className="max-w-4xl mx-auto px-6 py-16">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
          ...
          <div className="divide-y divide-neutral-800">
            {items.map(({ artwork }) => (
              <div key={artwork._id} className="flex gap-4 py-4">
                ...
                <p className="text-sm text-amber-400">R$ {artwork.price.toLocaleString('pt-BR')}</p>
              </div>
            ))}
          </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <button
            type="submit"
            className="w-full py-4 bg-amber-700 text-white text-sm tracking-widest uppercase hover:bg-amber-600 transition-colors mt-4"
          >
            Confirmar Pedido
          </button>
