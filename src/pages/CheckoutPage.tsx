  ...
  if (submitted) {
    return (
      <main className="max-w-2xl mx-auto px-6 py-32 text-center">
        <div className="w-12 h-12 border border-[var(--terra-h)] rounded-full flex items-center justify-center mx-auto mb-6">
        ...
        <p className="mt-4 text-[var(--cream-2)] leading-relaxed">
  ...
  return (
    <main className="max-w-4xl mx-auto px-6 py-16">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
        {/* Resumo */}
        <div>
          <p className="text-xs text-[var(--teal-500)] uppercase tracking-widest mb-6">Resumo do Pedido</p>
          <div className="divide-y divide-[var(--teal-600)]">
            {items.map(({ artwork }) => (
              <div key={artwork._id} className="flex gap-4 py-4">
                <div className="w-16 h-16 bg-[var(--teal-800)] overflow-hidden shrink-0">
                  <img
                ...
                <div className="flex-1">
                  ...
                  <p className="text-xs text-[var(--teal-500)] mt-1">{artwork.dimensions}</p>
                </div>
                <p className="text-sm text-[var(--peach)]">R$ {artwork.price.toLocaleString('pt-BR')}</p>
              </div>
            ))}
          </div>
          <div className="pt-4 border-t border-[var(--teal-600)] flex justify-between">
            <p className="text-[var(--cream-3)]">Total</p>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <p className="text-xs text-[var(--teal-500)] uppercase tracking-widest mb-6">Seus Dados</p>
          ...
          <button
            type="submit"
            className="w-full py-4 bg-[var(--terra)] text-white text-sm tracking-widest uppercase hover:bg-[var(--terra-h)] transition-colors mt-4"
          >
            Confirmar Pedido
          </button>

          <p className="text-xs text-[var(--teal-500)] text-center">
